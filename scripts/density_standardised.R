...
std_dat_summary %>% 
  ...
  summarise(mean_density = mean(density_std, na.rm = TRUE),
            se_density = plotrix::std.error(density_std, na.rm = TRUE),
            max_density = max(density_std, na.rm = TRUE),
            ...
            ) %>% select((max_density)) %>% print() # to get means
...
std_dat_summary_filtered$density_std %>% summary()
# Mutate rows with density_std values beyond the threshold
std_dat_summary_outlier_adj <- std_dat_summary %>%
  mutate(density_adj = if_else(density_std >= mean_density_std + threshold * sd_density_std, 320.41, density_std)
    )

# Plot density per mean shell length with outliers removed
density_plot_shell_length_filtered <- ggplot(std_dat_summary_outlier_adj, aes(x = mean_shell_length, y = density_adj)) +
  geom_point() +
  labs(x = "Mean Shell Length", y = "Density") +
...
std_dat_summary_outlier_adj$predicted_density <- predict(gam_model, newdata = std_dat_summary_outlier_adj)

# Plot the data and the fitted curve using the filtered data
density_plot_shell_length_gam <- ggplot(std_dat_summary_outlier_adj, aes(x = mean_shell_length, y = density_adj)) +
  geom_point(aes(),pch = 21, cex = 2.5, col = "goldenrod2", fill = "goldenrod2", alpha = 0.65) +
  geom_line(aes(y = predicted_density), color = "cornflowerblue", linewidth = 0.63) +
  labs(x = "Shell Length (mm)", y = "Density N animals/square meter") +
